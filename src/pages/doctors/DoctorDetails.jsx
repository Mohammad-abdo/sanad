    ...
    <div className="space-y-6">
      ...
      <div className="glass-card rounded-2xl p-6 bg-white border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-xl p-6 border border-gray-200 text-center">
            <div className="w-16 h-16 rounded-xl bg-orange-100 flex items-center justify-center mx-auto mb-3">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="px-4 py-3 text-xs font-semibold text-gray-700 uppercase text-right">المبلغ</th>
                  <th className="px-4 py-3 text-xs font-semibold text-gray-700 uppercase text-right">طريقة الدفع</th>
                  <th className="px-4 py-3 text-xs font-semibold text-gray-700 uppercase text-right">الحالة</th>
                  <th className="px-4 py-3 text-xs font-semibold text-gray-700 uppercase text-right">الحجز</th>
                  <th className="px-4 py-3 text-xs font-semibold text-gray-700 uppercase text-right">التاريخ</th>
                </tr>
              </thead>
                  ...
                  <tr key={payment.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">
                      {payment.method === 'BANK_ACCOUNT' ? 'حساب بنكي' :
                       payment.method === 'E_WALLET' ? 'محفظة إلكترونية' :
                       payment.method === 'PAYPAL' ? 'PayPal' :
                       payment.method === 'VODAFONE_CASH' ? 'فودافون كاش' :
                       payment.method === 'FAWRY' ? 'فوري' :
                       payment.method === 'INSTAPAY' ? 'انستا باي' : payment.method}
                    </td>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">
                      {payment.booking ? (
                        <div>
                          <p className="font-medium">{payment.booking.user?.username || 'مستخدم'}</p>
                          <p className="text-xs text-gray-500">
                            {payment.booking.scheduledAt 
                              ? new Date(payment.booking.scheduledAt).toLocaleDateString('ar-EG')
                              : '-'}
                          </p>
                        </div>
                      ) : '-'}
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="px-4 py-3 text-xs font-semibold text-gray-700 uppercase text-right">المبلغ</th>
                  <th className="px-4 py-3 text-xs font-semibold text-gray-700 uppercase text-right">طريقة السحب</th>
                  <th className="px-4 py-3 text-xs font-semibold text-gray-700 uppercase text-right">الحالة</th>
                  <th className="px-4 py-3 text-xs font-semibold text-gray-700 uppercase text-right">التاريخ</th>
                </tr>
              </thead>
                  ...
                  <tr key={withdrawal.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">
                      {withdrawal.method === 'BANK_ACCOUNT' ? 'حساب بنكي' :
                       withdrawal.method === 'E_WALLET' ? 'محفظة إلكترونية' :
                       withdrawal.method === 'PAYPAL' ? 'PayPal' :
                       withdrawal.method === 'VODAFONE_CASH' ? 'فودافون كاش' :
                       withdrawal.method === 'FAWRY' ? 'فوري' :
                       withdrawal.method === 'INSTAPAY' ? 'انستا باي' : withdrawal.method}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        ...
                        {withdrawal.status === 'COMPLETED' ? 'مكتمل' :
                         withdrawal.status === 'PENDING' ? 'معلق' :
                         withdrawal.status === 'PROCESSING' ? 'قيد المعالجة' :
                         withdrawal.status === 'REJECTED' ? 'مرفوض' : withdrawal.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-600">
